...
| D-6 | TypeScript's Document model cannot represent [§2.3](02-document-model.md#23-structural-invariants) D-5's `integer`/`number` distinction independent of a schema — JS has one numeric type, so `Node`'s scalar union carries no kind tag for numbers, and `valueKind`/`matchesKind` derive `"integer"` vs `"number"` from `Number.isInteger(v)` (shape, not source kind). Confirmed and already documented in `omnist-ts`'s own `docs/python-parity.md` #1 (issue #3 there): `matchesKind(1.0, "integer")` is `true` in TypeScript, `false` in Python. This is a deliberate, target-language-driven design choice there, not drift — but it means any conformance vector whose `document` input depends on this distinction (e.g. `test-suite/validate/scalar-kinds/number-does-not-satisfy-integer-even-when-whole`) cannot be faithfully constructed in TypeScript's model and MUST be reported `skip`, never `fail` or a forced `pass`, by its conformance runner. Rust is unaffected (native `i64`/`f64` distinction). **Reconsidered 2026-08-10 alongside D-9: this may not be a genuine language limitation after all.** D-9 requires TypeScript's `integer` to move off `number` onto `bigint` regardless (for arbitrary-precision correctness); once that migration happens, `typeof v === "bigint"` vs `typeof v === "number"` is a real, native kind tag — no shape-guessing needed, and this entry would close as a side effect rather than needing its own fix. Not closing this entry yet since the `bigint` migration (`omnist-ts#98`) hasn't landed, but flagging explicitly so the fix isn't scoped as "just add bigint for large integers" while leaving D-6 open unnecessarily — the same `bigint` type answers both. | Open, TypeScript-only, **pending re-evaluation after `omnist-ts#98` lands** (see note above) rather than accepted as permanent. Tracked alongside `omnist-ts` issue #85. |
