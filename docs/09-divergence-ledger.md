...
| D-3 | XML attributes and namespace prefixes are dropped silently, with no adjustment reported. [§8.3.8](08-conformance-and-errors.md#838-format--codec-adjustments) defines codes for reporting them. Why this matters in practice, concretely rather than abstractly: [`docs/formats/yaml.md`](formats/yaml.md)'s "Norway problem" is the closest real-world case in the ecosystem of a codec doing something a document author never expected — an unquoted `on:` key silently becoming the boolean `true` under YAML 1.1. That case is *caught*, loudly, as a `DocumentError`, because a boolean key can't become a Document label at all. D-3's XML case is the same class of surprise with no error at all — the attribute is simply gone, and a caller has no way to learn it happened short of reading the source text themselves. | Open. Reporting is a behavior change; vector first. |
